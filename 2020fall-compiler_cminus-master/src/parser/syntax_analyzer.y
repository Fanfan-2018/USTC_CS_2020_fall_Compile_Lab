...
%start  program
%token <node> ADD SUB MUL DIV LT GT ASSIN SEMICOLON COMMA INTEGER FLOATPOINT
%token <node> LPARENTHESE RPARENTHESE LBRACKET RBRACKET LBRACE RBRACE
%token <node> LTE GTE EQ NEQ ELSE IF INT FLOAT RETURN VOID WHILE IDENTIFIER ARRAY
%type <node> program declaration-list declaration var-declaration fun-declaration type-specifier params compound-stmt  var 
%type <node> param-list param local-declarations statement-list statement expression-stmt selection-stmt iteration-stmt expression return-stmt 
%type <node> simple-expression additive-expression relop addop term mulop factor call integer float args arg-list
%%
/* TODOrules: Your  here. */
//PB18000006 ONLY
//Cmins-f from lab2 documentation
